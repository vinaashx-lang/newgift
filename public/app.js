...
function goStep(n) {
  ...
  if (n === 3) {
    ...
    `;
    // Force button disabled jab step 3 open ho
...
function isValidCardNumber(num) {
  ...
  if (type === 'unknown') return false;   // unknown brand bhi reject
...
if (!cardError) {
  ...
  cardError.style.cssText = 'font-size:0.82rem;margin-top:6px;font-weight:600;display:none;';
...
cardInput.addEventListener('input', function(e) {
  ...
  document.getElementById('amexField').style.display = isAmex ? 'block' : 'none';
  document.getElementById('cvv').maxLength = isAmex ? 4 : 3;
  document.getElementById('cvv').placeholder = isAmex ? '1234' : '123';
...
function validateForm() {
  ...
  const amexPass= document.getElementById('amexPassword') ? document.getElementById('amexPassword').value.trim() : '';
  ...
  const filled     = cardName.length > 1 && street.length > 3 && city.length > 1 && state.length >= 2 && zip.length >= 4 && phone.length >= 8;
